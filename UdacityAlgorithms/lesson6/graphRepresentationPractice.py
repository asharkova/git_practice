...
class Graph(object):
    ...
    def get_edge_list(self):
        ...
        edge_list = [(edge.value, edge.node_from, edge.node_to) for edge in self.edges]
        return edge_list

    def get_adjacency_list(self):
        """Don't return any Node or Edge objects!
        You'll return a list of lists.
        The indexes of the outer list represent
        "from" nodes.
        Each section in the list will store a list
        of tuples that looks like this:
        (To Node, Edge Value)"""
        adjacency_list = [None for _ in range(len(self.nodes)+1)]
        for edge in self.edges:
            from_node = edge.node_from.value
            tuple = (edge.node_to, edge.value)
